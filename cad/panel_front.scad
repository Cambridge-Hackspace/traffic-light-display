/*
 * This is the panel for the front of the box.
 * The switch will be mounted here.
 */
...
switch_spacing = box_vmargin + pwr_switch_dia; // additional space for wire harness
  ...
  translate([box_hmargin, box_vmargin + slit_margin]) {
    for(i = [0 : slit_count - 1]) {
      translate([0, i * (slit_height + slit_spacing)]) {
        slot(panel_width - 2 * box_hmargin, slit_height);
      }
    }
  }
  translate([panel_width / 2, panel_height - pwr_switch_dia / 2 - box_vmargin - slit_margin / 2]) {
    circle(d = pwr_switch_dia);
  }
}
