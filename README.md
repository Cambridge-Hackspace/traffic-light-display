...
The portal — the settings page, and the update endpoint with it — is behind a
password. A light that has never had one set stays open and says so in red at
the top of its page; firmware upload is the one thing it will not do until a
password exists. Set one under **Portal access** on the page.

The push script asks for the password when the light wants one. To stop being
asked, put it in `~/.netrc` (not in this repo):

```
machine 192.168.1.110 login admin password <the password>
```
...
The password is never put on a command line, so it cannot end up in your shell
history or in `ps`.
...
| `wrong password` | see `~/.netrc` above |
...
| `came back running the same image it had before` | it rolled back — read the serial log with `espflash monitor` |
